...
def create_fixture_user_groups() -> None:
    ...
    if proc.stdout != b'[\n]\n':
        with open('market/fixtures/user_groups.json', 'wb') as file:
            file.write(proc.stdout)
    else:
        basic_group = """[
    {
    ...
    """
        with open('market/fixtures/user_groups.json', 'w') as file:
...
def update_user_groups_permissions(sender, **kwargs):
    ...
    if not checking_for_permissions_updates():
        with (open('market/fixtures/permissions.json', 'r') as file):
        ...
        with open('market/fixtures/user_groups.json', 'r') as file:
            file_json = json.load(file)
