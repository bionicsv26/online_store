...
def change_seller_product_discount(sender, instance, pk_set, action, **kwargs):
    ...
    """
    undiscounted_discount = Discount.objects.filter(type__name='undiscounted').first()
    if not undiscounted_discount:
        return
    ...
    if action == 'pre_remove':
        seller_products.update(discount=undiscounted_discount)
